...
const log = (text: string) => {
 ...
 const toggleChecklistItem = (clId: string, index: number) => {
  setDraft(d => {
    const checklist = d.checklists.find(c => c.id === clId);
    if (!checklist) return d;

    const item = checklist.items[index];
    const newStatus = !item.done;

    // Remove previous log for this checklist item
    setPendingActivity(prev =>
      prev.filter(
        log => !log.text.includes(`Checklist item "${item.text}"`)
      )
    );
  ...
  setDraft(d => {
    ...
    if (d.attachments.includes(file.name)) {
      return d; // do nothing if already exists
    }
...
if (draft.startDate && draft.dueDate) {
  if (draft.dueDate < draft.startDate) {
    newErrors.date = "Due date cannot be earlier than Start date";
  }
}

  draft.checklists.forEach((cl, ci) => {
    cl.items.forEach((item, ii) => {
      if (!item.text.trim()) {
        newErrors[`check-${ci}-${ii}`] = "Checklist item cannot be empty";
      }
    });
  });
...
const saveAll = () => {
  ...
  onSave({
    ...card,
    ...draft,
    activity: [...draft.activity, ...pendingActivity],
    checklists: draft.checklists,
    checklist: undefined
  });
  setPendingActivity([]); // clear logs
